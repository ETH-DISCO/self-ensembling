...
# see: https://github.com/pytorch/vision/blob/main/torchvision/transforms/_presets.py
...
class ResNet_conv(ResNet):
    # - remove the final fully connected layer (self.fc)
    # - change forward pass to return flattened outputs from multiple layers of the network
...
class ResNet_conv_with_linear_probes(ResNet):
    # returns intermediate outputs of the network as well as the final output
    # - remove the final fully connected layer (self.fc)
    # - change forward pass to return flattened outputs from multiple layers of the network
    # - multiple linear "probe" layers, one for each intermediate output of the network.
    # - a custom forward pass that applies these probes to the intermediate outputs.
    ...
    def __init__(self, *args, num_classes=10, **kwargs) -> None:
        ...
        self.fc_layers = nn.ModuleList([nn.Linear(output_length, num_classes) for output_length in output_lengths])
...
def get_cross_max_consensus_logits(outputs: torch.Tensor, k: int, self_assemble_mode: bool = False) -> torch.Tensor:
    ...
    assert Y.dtype == torch.float16  # assert float
    return Y
