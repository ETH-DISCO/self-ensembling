# `srun --mem=10GB--gres=gpu:1 --nodelist=tikgpu04 --pty bash`
...
# dispatch
sbatch --output=$(pwd)/%j.out --error=$(pwd)/%j.err --nodelist=$(hostname) --mem=150G --nodes=1 --gres=gpu:1 --wrap="bash -c 'source /itet-stor/${USER}/net_scratch/conda/etc/profile.d/conda.sh && conda activate con && python3 $(pwd)/tmp/resnet.py'"
